...
## store the result
dt.result <- as.data.table(matrix(rep(0, 5 * k * length(cols.features)), k * length(cols.features)))
setnames(dt.result, c("round", "params", "cv_num", "result.dval", "resul.valid"))
df.result <- as.data.frame(dt.result) # df is easier to insert rows

dt.summary <- as.data.table(matrix(rep(0, 10 * length(cols.features)), length(cols.features)))
setnames(dt.summary, c("round", "params", "mean.dval", "max.dval", "min.dval", "sd.dval", "mean.valid", "max.valid", "min.vaild", "sd.valid"))
...
df.summary
save(dt.summary, file = "data/cv_results/single_xgb_all_features.RData")
